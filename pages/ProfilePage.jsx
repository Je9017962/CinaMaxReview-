      ...
      <div style={{ minHeight: '100vh', background: 'var(--color-bg)', fontFamily: 'var(--font-body)' }}>
        ...
        <div style={{ maxWidth: 520, margin: '80px auto', padding: '0 28px', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--color-text-muted)', fontSize: 16, marginBottom: 28 }}>
            Create an account to track your reviews, build your watchlist, and join the community.
          </p>
          <button
...
function ProfileContent({ user, updateProfile, signOut, navigate }) {
  ...
  const avgRating = allReviews.length
    ...
    : '—'
  const topGenres = getTopGenres(allReviews, allMovies)
  ...
  return (
    <main style={{ maxWidth: 860, margin: '0 auto', padding: '40px 28px 80px' }}>
      ...
      {/* ── Profile card ── */}
      <div style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: 20, padding: '32px', marginBottom: 28, display: 'flex', gap: 28, alignItems: 'flex-start', flexWrap: 'wrap' }}>
          ...
          {editing ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={handleSave} style={primaryBtnStyle}>SAVE PROFILE</button>
      ...
      {allReviews.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {allReviews.length > 5 && (
            <Link to="/my-submissions" style={{ textAlign: 'center', display: 'block', padding: '14px', color: 'var(--color-accent)', fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: 14, background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: 12 }}>
...
const inputStyle = {
  ...
  border: '1.5px solid var(--color-input-border)', borderRadius: 10,
